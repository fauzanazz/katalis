...
import { analyzeStoryFn } from "@/lib/server/discovery";
...
export function StoryPrompt({
  ...
  const submitStoryAnalysis = useCallback(
    async (text: string, submissionType: "text" | "audio") => {
      ...
      try {
        const res = await analyzeStoryFn({
          data: {
            storyText: text,
        ...
        // content_blocked was returned as HTTP 200 + redirect:true in the old route;
        // preserve that branch as ai_failure since the old fetch() only saw !response.ok
        // for hard errors — blocked was surfaced upstream by the page, not here.
        ...
        onAnalysisComplete(res as unknown as AnalysisOutput);
      } catch {
        onError("network");
      } finally {
        setIsSubmitting(false);
  ...
  const handleTextSubmit = useCallback(() => {
    if (storyText.length < STORY_MIN_LENGTH) {
      setValidationError(m.discover_story_validation_tooShort({ min: STORY_MIN_LENGTH }));
      return;
    }
    if (storyText.length > STORY_MAX_LENGTH) {
      setValidationError(m.discover_story_validation_tooLong({ max: STORY_MAX_LENGTH }));
      return;
    }
    submitStoryAnalysis(storyText, "text");
  }, [storyText, submitStoryAnalysis]);
      ...
      <div className="flex flex-col gap-6">
        ...
        <div>
          <h2 className="mb-3 text-center text-lg font-semibold text-ink">
            {m.discover_story_imagePromptTitle()}
          </h2>
          <p className="mb-4 text-center text-sm text-muted-foreground">
            {m.discover_story_imagePromptSubtitle()}
          </p>
          <div
            className="grid grid-cols-3 gap-3"
            role="group"
            aria-label={m.discover_story_imageGroupLabel()}
          >
            {images.map((image) => (
              <button
                key={image.id}
                type="button"
                aria-label={`${imgAlt(image)} — ${m.discover_story_imageExpandLabel()}`}
        ...
        {inputMode === "text" && (
          <div className="flex flex-col gap-3">
            ...
            <textarea
              ...
              placeholder={m.discover_story_textPlaceholder()}
              disabled={isSubmitting}
              aria-label={m.discover_story_textLabel()}
        ...
        {inputMode === "audio" && (
          <div className="flex flex-col gap-4">
            <p className="text-center text-sm text-muted-foreground">
              {m.discover_story_audioInstructions()}
            </p>
            <AudioRecorder
