...
const DEFAULT_GLOW_COLOR = "246, 169, 38"; // yellow-sun-deep
...
export type MagicBentoItem = {
  ...
  title: string;
  description: string;
  /** Tailwind text-* color class for the accent bar above the title. */
...
function ParticleCard({
  ...
  return (
    <div
      ref={cardRef}
      data-size={dataSize}
      className={cn("particle-container magic-bento-card", className)}
      style={style}
    >
      {children}
    </div>
        ...
        {items.map((item) => {
            ...
            <>
              ...
              <div className="magic-bento-card__content">
                {item.accentClassName ? (
                  <span
                    aria-hidden
                    className={cn("magic-bento-card__accent", item.accentClassName)}
                  />
                ) : null}
          ...
          return (
            <div
              key={item.id}
              data-size={item.size}
              className={cn("magic-bento-card", className)}
              style={style}
            >
              {inner}
            </div>
