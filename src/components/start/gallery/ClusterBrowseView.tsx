...
import { clusterGalleryEntriesFn } from "@/lib/server/gallery";
...
export function ClusterBrowseView({
  ...
  const fetchClusters = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await clusterGalleryEntriesFn();
      if (!res.ok) {
        throw new Error(res.message);
      }
      setClusters((res.clusters as ClusterData[]) ?? []);
    } catch (err) {
  ...
  if (error) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-4">
        <p className="text-sm text-red-800">{m.gallery_clusters_error()}</p>
        <button
          onClick={fetchClusters}
          className="mt-2 text-sm font-medium text-red-700 underline hover:text-red-900"
        >
          {m.gallery_clusters_viewAll()}
        </button>
      </div>
    );
  }
  ...
  if (clusters.length === 0) {
    return (
      <div className="rounded-lg border bg-muted/30 p-6 text-center">
        <p className="text-sm text-muted-foreground">{m.gallery_clusters_empty()}</p>
      </div>
    );
  }
  ...
  if (selectedCluster) {
    return (
      <div role="region" aria-label={m.gallery_clusters_clusterDetail({ label: selectedCluster.label })}>
        <button
        ...
        >
          ...
          {m.gallery_clusters_backToClusters()}
        </button>
  ...
  // Cluster list view
  return (
    <div role="region" aria-label={m.gallery_clusters_ariaLabel()}>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {clusters.map((cluster) => (
          <button
            ...
            aria-label={`${cluster.label} - ${m.gallery_clusters_worksCount({ count: cluster.entries.length })}`}
          >
            {/* Cluster header */}
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>{m.gallery_clusters_worksCount({ count: cluster.entries.length })}</span>
