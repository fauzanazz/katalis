...
export const Route = createFileRoute("/$locale/quest/preview/")({
    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-8 sm:py-12">
      ...
      <div className="mb-6 text-center">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-ink sm:text-4xl">
          {m.quest_preview_title()}
        </h1>
        <p className="mt-2 text-muted-foreground">{m.quest_preview_subtitle()}</p>
        <p className="mt-1 text-sm text-muted-foreground">
          {m.quest_preview_missionCount({ count: quest.missions.length })}
        </p>
      </div>
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              <span className="inline-flex items-center rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-700">
                {m.quest_preview_dayLabel({ day: mission.day })}
              </span>
