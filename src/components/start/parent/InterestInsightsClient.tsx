...
import { getChildInterestInsightsFn } from "@/lib/server/parent-interests";
...
function fetchInsights(
  ...
  let cancelled = false;
  getChildInterestInsightsFn({ data: { childId } })
    .then((result) => {
      if (cancelled) return;
      if (result.ok) {
        onSuccess(result.insights as InterestInsightsData);
      } else {
        console.error("InterestInsightsClient:", result.error);
        onError();
      }
    })
    .catch((err: unknown) => {
