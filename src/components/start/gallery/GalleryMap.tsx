...
export function GalleryMap({ data, isLoading }: GalleryMapProps) {
  ...
  if (isLoading) {
    return (
      <div
        ...
        role="status"
        aria-label={m.gallery_map_loading()}
      >
        <div className="flex flex-col items-center gap-3">
          <div className="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent" />
          <p className="text-sm text-muted-foreground">{m.gallery_map_loading()}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      ...
      role="application"
      aria-label={m.gallery_map_ariaLabel()}
    >
      {/* Map loading overlay */}
      {!mapLoaded && !mapError && (
      ...
      {mapError && (
        <div className="flex h-[500px] w-full items-center justify-center rounded-lg bg-muted/50 lg:h-[600px]">
          <div className="text-center">
            <p className="text-sm text-muted-foreground">{m.gallery_map_error()}</p>
      ...
      {categories.length > 0 && (
        <div
          ...
          role="list"
          aria-label={m.gallery_map_legendLabel()}
        >
  ...
  if (!data || data.features.length === 0) {
    return (
      <div className="flex h-[400px] items-center justify-center rounded-lg bg-muted/50">
        <p className="text-sm text-muted-foreground">{m.gallery_map_noEntries()}</p>
      </div>
    );
  }
