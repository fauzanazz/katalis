...
import { submitReflectionFn } from "@/lib/server/mentor";
...
export function ReflectionCard({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const res = await submitReflectionFn({
        data: { questId, missionDay, type: "text", content: content.trim() },
      });
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <p className="text-sm">{m.mentor_reflection_alreadyExists()}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-xl border bg-card p-4 shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-foreground">{m.mentor_reflection_title()}</h3>
        </div>
        <p className="mt-2 text-sm text-muted-foreground">{m.mentor_reflection_encouragement()}</p>
      </div>
    );
  }
    ...
    <div className="rounded-xl border bg-card shadow-sm">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-foreground">{m.mentor_reflection_title()}</h3>
            <p className="text-xs text-muted-foreground">{m.mentor_reflection_subtitle()}</p>
          </div>
        </div>
        {expanded ? (
      ...
      {expanded && (
        <div className="border-t px-4 pb-4 pt-3">
          <p className="mb-3 text-sm text-muted-foreground">
            {m.mentor_reflection_encouragement()}
          </p>

          <textarea
            value={content}
            onChange={(e) => setContent(e.target.value)}
            placeholder={m.mentor_reflection_textPlaceholder()}
            rows={4}
          ...
          >
            ...
            {m.mentor_reflection_submitButton()}
          </Button>
        </div>
      )}
    </div>
