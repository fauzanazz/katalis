...
const ScrollStack = ({
  ...
  const measureDocumentOffset = useCallback((element: HTMLElement) => {
    let top = 0;
  ...
  const recomputeOffsets = useCallback(() => {
    cardOffsetsRef.current = cardsRef.current.map((card) =>
      useWindowScroll ? measureDocumentOffset(card) : card.offsetTop,
    );
    ...
    endOffsetRef.current = endEl
      ? useWindowScroll
        ? measureDocumentOffset(endEl)
        : endEl.offsetTop
      : 0;
  }, [measureDocumentOffset, useWindowScroll]);
