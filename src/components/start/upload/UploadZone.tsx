...
export function UploadZone({
  ...
  const validateFile = useCallback((file: File): string | null => {
    if (!(ACCEPTED_IMAGE_TYPES as readonly string[]).includes(file.type)) {
      return m.discover_upload_errors_invalidType();
    }
    if (file.size > MAX_IMAGE_SIZE_BYTES) {
      return m.discover_upload_errors_tooLarge();
    }
    return null;
  }, []);
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      try {
        // Step 1: Get presigned URL via server fn
        setProgress(10);
        ...
        if (!presignedRes.ok) {
          const code = presignedRes.error;
          if (code === "rate_limited") {
            throw new Error(m.discover_upload_errors_uploadFailed());
          } else if (code === "invalid" || code === "invalid_type") {
            throw new Error(m.discover_upload_errors_invalidType());
          }
          throw new Error(m.discover_upload_errors_uploadFailed());
        }
        ...
        // Step 2: Upload file directly to presigned URL (unchanged)
        ...
        // Step 3: Confirm upload via server fn
        ...
        const completeData = completeRes;
        setProgress(100);
        ...
        const message =
          err instanceof TypeError
            ? m.discover_upload_networkError()
            : err instanceof Error
              ? err.message
              : m.discover_upload_errors_uploadFailed();
        handleError(message);
      } finally {
        abortControllerRef.current = null;
      }
    },
    [clearError, validateFile, handleError, onUploadComplete, storageKey],
  );
  ...
  const handleDrop = useCallback(
    (e: DragEvent<HTMLDivElement>) => {
      ...
      if (files.length > 1) {
        handleError(m.discover_upload_errors_multipleFiles());
        return;
      }
    ...
    [disabled, uploadState, handleError, uploadFile],
  );
  ...
  }, [disabled, handleError, uploadFile, uploadState]);
    ...
    return (
      <div className="flex flex-col items-center gap-4 rounded-lg border-2 border-green-300 bg-green-50 p-6">
        <div className="relative">
          <img
            src={preview}
            alt={m.discover_upload_altPreview()}
            className="max-h-48 max-w-full rounded-lg object-contain"
          />
        </div>
        <p className="text-sm font-medium text-green-700">
          {m.discover_upload_uploadComplete()}
        </p>
  ...
  return (
    <div className="relative flex flex-col gap-4 pt-12">
      ...
      <div
        ...
        aria-label={m.discover_upload_dropzone()}
        ...
        {uploadState === "uploading" ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-medium text-foreground">
              {m.discover_upload_uploading()}
            </p>
            {/* Progress bar */}
            <div className="w-full max-w-xs">
              <div
                role="progressbar"
                aria-label={m.discover_progress_label()}
              ...
              <p className="mt-1 text-center text-xs text-zinc-500">
                {m.discover_progress_percent({ percent: progress })}
              </p>
            </div>
            <Button
              variant="ghost"
              ...
              type="button"
            >
              {m.discover_upload_cancelUpload()}
            </Button>
          </div>
        ) : (
            ...
            {isDragOver ? (
              <>
                ...
                <p className="text-base font-semibold text-blue-600">
                  {m.discover_upload_dropzoneActive()}
                </p>
              </>
            ) : (
              <>
                ...
                <div className="flex flex-col items-center gap-1">
                  <p className="hidden text-base font-bold text-foreground sm:block">
                    {m.discover_upload_dropzone()}
                  </p>
                  <p className="hidden text-sm text-muted-foreground sm:block">
                    {m.discover_upload_clickToUpload()}
                  </p>
                  {/* Mobile text */}
                  <p className="text-base font-bold text-foreground sm:hidden">
                    {m.discover_upload_tapToUpload()}
                  </p>
                </div>
              </>
            )}
            <p className="text-xs text-zinc-400">
              {m.discover_upload_acceptedFormats()}
            </p>
          </div>
        )}
      </div>
      ...
      >
        ...
        {m.discover_upload_cameraCapture()}
      </Button>
