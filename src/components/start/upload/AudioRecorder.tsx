...
export function AudioRecorder({
  ...
  const startRecording = useCallback(async () => {
    ...
    if (typeof MediaRecorder === "undefined") {
      handleError(m.discover_audio_errors_unsupported());
      return;
    }
    ...
    try {
      ...
      recorder.onerror = () => {
        handleError(m.discover_audio_errors_recordingFailed());
        stream.getTracks().forEach((track) => track.stop());
        streamRef.current = null;
      };
      ...
      if (err instanceof DOMException && err.name === "NotAllowedError") {
        handleError(m.discover_audio_errors_permissionDenied());
      } else {
        handleError(m.discover_audio_errors_recordingFailed());
      }
    }
  ...
  const uploadAudio = useCallback(
    async (blob: Blob, filename: string, contentType: string) => {
      ...
      try {
        ...
        setProgress(10);
        const presignedRes = await getPresignedUploadUrlFn({ data: { filename, contentType } });

        if (!presignedRes.ok) {
          throw new Error(presignedRes.message || m.discover_audio_errors_recordingFailed());
        }
        ...
        if (!uploadRes.ok) {
          throw new Error(m.discover_audio_errors_recordingFailed());
        }
        ...
        const message =
          err instanceof TypeError
            ? m.discover_audio_errors_recordingFailed()
  ...
  const handleFileUpload = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!(ACCEPTED_AUDIO_TYPES as readonly string[]).includes(file.type)) {
        handleError(m.discover_audio_errors_invalidType());
        e.target.value = "";
        return;
      }
      ...
      if (file.size > MAX_AUDIO_SIZE_BYTES) {
        handleError(m.discover_audio_errors_tooLarge());
        e.target.value = "";
        return;
      }
    ...
    return (
      <div className="flex flex-col items-center gap-4 rounded-lg border-2 border-green-300 bg-green-50 p-6">
        <p className="text-sm font-medium text-green-700">
          {m.discover_audio_uploadComplete()}
        </p>
        {audioUrl && (
          <audio
            ref={audioElementRef}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col items-center gap-4 rounded-lg border-2 border-border bg-muted p-6">
        {recorderState === "idle" || recorderState === "requesting-permission" || recorderState === "error" ? (
          <div className="flex flex-col items-center gap-3">
            ...
            >
              ...
              {recorderState === "requesting-permission"
                ? "..."
                : m.discover_audio_startRecording()}
            </Button>
          </div>
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-medium text-red-600">
              {m.discover_audio_recording()} {formatDuration(duration)}
            </p>
            <Button
            ...
            >
              ...
              {m.discover_audio_stopRecording()}
            </Button>
          </div>
        ) : recorderState === "stopped" ? (
          <div className="flex flex-col items-center gap-3">
            <p className="text-sm font-medium text-foreground">
              {m.discover_audio_recordingComplete()} ({formatDuration(duration)})
            </p>
            {audioUrl && (
              <audio
                ref={audioElementRef}
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {m.discover_audio_uploading().replace("...", "")}
              </Button>
              <Button
        ...
        <p className="text-xs text-zinc-400">
          {m.discover_audio_acceptedFormats()}
        </p>
      </div>
