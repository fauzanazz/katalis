/**
 * Google Cloud Vertex AI provider for multi-modal AI tasks.
 * Supports image analysis, text generation, and moderation via Gemini models.
 * 
...
async function generateContent<T>(
  ...
  try {
    ...
    return parse(JSON.parse(result.text));
  } catch (error) {
...
Design missions that connect their dream with their talents, using materials available in their local environment. Make it practical, fun, and progressively challenging.`;
  ...
  async moderateText(content: string): Promise<ModerationResult> {
    try {
      ...
      console.error("[Moderation] Vertex AI text moderation error:", error);
      return {
        allowed: false,
        status: "flagged",
        category: undefined,
  ...
  async moderateImage(imageUrl: string): Promise<ModerationResult> {
    try {
      ...
      const userParts: Part[] = [
        { text: "Analyze this image for child safety concerns:" } as Part,
        {
          inline_data: {
            mime_type: mimeType,
            data: base64Image,
      ...
      console.log("[Moderation] Vertex AI image result:", {
      ...
      console.error("[Moderation] Vertex AI image moderation error:", error);
      return {
        allowed: true,
        status: "error",
        category: undefined,
